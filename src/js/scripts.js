const handleAmbassadorForm = ({ redirectUrl, klaviyoA, klaviyoG }) => {
  ...
  const checkInput = (input) => {
    ...
    if (input.type === "tel" && !iti.isValidNumber() && input.value.trim().length > 0) {
      if (!input.nextElementSibling?.classList.contains("error-text")) input.classList.add("error");
  ...
  const svgButton = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="14.5962" y="4.59619" width="1" height="14" transform="rotate(45 14.5962 4.59619)" fill="white" /><rect x="15.3033" y="14.4957" width="1" height="14" transform="rotate(135 15.3033 14.4957)" fill="white" /></svg>';
  ...
  const options = '<option value="" hidden>How many followers do you have?</option><option value="0 - 1000">0 - 1000 </option><option value="1001 - 2500">1001 - 2500</option><option value="2501 - 5000">2501 - 5000</option><option value="5001 - 10000">5001 - 10000</option><option value="10001+">10000+</option>';
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      formData.append("amb_source", `${sourceField}${utm_medium}`);
    } catch {
      body.source = utm_medium;
      formData.append("amb_source", utm_medium);
    }
    try {
